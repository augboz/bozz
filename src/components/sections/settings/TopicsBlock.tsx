...
// A small palette of cool/neutral accent colours that play nicely against
// the dark themes. The user can pick any one; clicking the swatch cycles.
const COLOR_PRESETS = [
  '#c9a8d4', '#a1bdc7', '#b8c7a1', '#d4b896', '#c7a1a1',
  '#bfa8c9', '#a8c7b4', '#d0b89a', '#b8b0c4', '#a8b8c4',
  '#7da7d9', '#e8a55a',
];
    ...
    <div>
      {topics.length === 0 && (
        <div style={{
          ...
          You don't have any topics yet. Add one to start tracking tasks — pick a name
          (e.g. <em>Music</em>, <em>CV</em>, <em>Job hunt</em>), then list the stages
          you want it to have.
        </div>
      )}
...
function TopicCard({ t, topic, expanded, onToggle, onChange, onDelete, onMoveUp, onMoveDown }: {
  ...
  const removeStage = (idx: number) => {
    if (topic.stages.length <= 1) return; // need at least one stage
  ...
  return (
    <div style={{
      ...
      background: t.todoBg, overflow: 'hidden',
    }}>
      {/* Header row */}
      <button
        onClick={onToggle}
      ...
      >
        ...
        <span
          ...
          style={{
            ...
            background: topic.color, flexShrink: 0, cursor: 'pointer',
            border: `1px solid ${t.border}`,
          }}
        />
        <span style={{
          flex: 1, minWidth: 0, fontSize: '0.9rem', color: t.text,
        ...
        <span style={{ fontSize: '0.68rem', color: t.textDim, flexShrink: 0 }}>
          {topic.stages.length} stage{topic.stages.length !== 1 ? 's' : ''}
          {' · '}
      ...
      {expanded && (
        <div style={{
          ...
          {/* Stages */}
          <div>
            <label style={lbl(t)}>Stages</label>
                ...
                <div key={s.id} style={{
                  ...
                  <span
                    ...
                    style={{
                      ...
                      background: s.color ?? t.textDim, cursor: 'pointer',
                  ...
                  <label style={{
                    ...
                    fontSize: '0.68rem', color: t.textMuted, cursor: 'pointer',
                  }}>
                    <input
                      type="checkbox"
                      checked={!!s.done}
                      onChange={(e) => setStage(i, { done: e.target.checked })}
                      style={{ accentColor: t.doneAccent }}
                    />
                    done
                  </label>
                  <button
                  ...
                  <button
                    onClick={() => moveStage(i, 1)}
                    disabled={i === topic.stages.length - 1}
                    aria-label="Move stage down"
                    style={miniBtn(t, i === topic.stages.length - 1)}
                  >↓</button>
                  <button
                    onClick={() => removeStage(i)}
                    disabled={topic.stages.length <= 1}
                    aria-label="Remove stage"
                    style={miniBtn(t, topic.stages.length <= 1)}
                  >
            ...
            <div style={{ display: 'flex', gap: '0.4rem', marginTop: '0.4rem' }}>
              <input
                ...
                style={inp}
              />
              <button onClick={addStage} style={smallBtn(t)}>
          ...
          <div>
            <label style={lbl(t)}>
              Voice keywords
              <span style={{ color: t.textDim, fontWeight: 400, marginLeft: '0.35rem' }}>
                (optional — used to route voice + quick capture)
              </span>
            </label>
            <div style={{
              display: 'flex', flexWrap: 'wrap', gap: '0.35rem',
              marginBottom: topic.keywords.length > 0 ? '0.4rem' : 0,
            }}>
              {topic.keywords.map(kw => (
                <span key={kw} style={{
                  display: 'inline-flex', alignItems: 'center', gap: '0.25rem',
                  background: t.input, border: `1px solid ${t.border}`,
                  borderRadius: '999px', padding: '0.15rem 0.5rem 0.15rem 0.65rem',
            ...
            <div style={{ display: 'flex', gap: '0.4rem' }}>
              <input
                ...
                style={inp}
              />
              <button onClick={addKeyword} style={smallBtn(t)}>
                <Plus size={12} strokeWidth={1.6} /> Add
              </button>
            </div>
          </div>
          ...
          <div style={{
            ...
            gap: '0.4rem', borderTop: `1px solid ${t.border}`, paddingTop: '0.55rem',
          }}>
            <div style={{ display: 'flex', gap: '0.3rem' }}>
              {onMoveUp && <button onClick={onMoveUp} style={smallBtn(t)}>↑ Move up</button>}
              {onMoveDown && <button onClick={onMoveDown} style={smallBtn(t)}>↓ Move down</button>}
            </div>
...
const lbl = (t: Theme): React.CSSProperties => ({
  ...
  color: t.textMuted, marginBottom: '0.35rem',
