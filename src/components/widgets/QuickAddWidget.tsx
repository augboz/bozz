...
import type { TaskListKey } from '../../lib/types';
...
export default function QuickAddWidget({ ctx }: { ctx: WidgetCtx }) {
  ...
  const submit = () => {
    ...
    addTask(list, v, deadline);
    setText(''); setDeadline(null);
    setFlash(`added to ${list}`);
    inputRef.current?.focus();
    window.setTimeout(() => setFlash(null), 1800);
  };

  const onVoice = (transcript: string) => {
    // Drop in the transcript and let the user confirm with Enter / Add.
  ...
  return (
    <Widget t={t} accent={ACCENT}>
      <WidgetHeader label="Quick Add" accent={ACCENT} t={t} icon={Zap} />
      ...
      <div style={{
        ...
        <ChoicePicker
          t={t}
          value={list}
          onChange={v => setList(v as TaskListKey)}
          options={LISTS}
          size="sm"
          minWidth={108}
        />
